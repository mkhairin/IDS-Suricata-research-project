    ...
    try:
        ...
        if output_excel:
            ...
            cols = [
                ...
                'dest_port',
                'proto',
                # Tambahan: Biar tahu protokol aplikasinya (misal: http, ssh, ftp)
                'app_proto',
                ...
                # --- 3. HTTP (Bukti SQLMap, XSS, Traversal) ---
                'http.http_method',
                'http.url',
                # Tambahan PENTING: Cek kode 200 (sukses) atau 403 (gagal/blokir)
                'http.status',
                'http.http_user_agent',
                'http.http_content_type',

                # --- 4. SSH (Bukti Hydra) ---
                # Tambahan: Biasanya Hydra meninggalkan jejak versi disini
                'ssh.client.software_version',

                # --- 5. FLOW STATS (Bukti DoS & Nmap) ---
                # Tambahan: Jumlah paket ke server (Cek banjir DoS)
                'flow.pkts_toserver',
                'flow.pkts_toclient',   # Tambahan: Jumlah paket balasan
                # Tambahan: Ukuran data (Cek anomali ukuran paket)
                'flow.bytes_toserver',

                # --- 6. FILE (Bukti RCE/Malware) ---
                'fileinfo.filename'
            ]
